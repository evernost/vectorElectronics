...
We don't know in what region the diode operates at yet, so $a$ and $b$ are unknown, but that is not an issue: the equation stays the same so we can carry on and solve for $V_D$ regardless:
$$
...
Which is a very common result, and this whole calculation justifies the usual diode approximation: the output does not depend on the polarisation voltage $V$ anymore.
...
Another variation on a theme of a Diode and Resistor circuit. This time, we have 2 resistors, and we don't neglect the output current anymore.
...
The derivation is more straightforward as there is no feedback involved here.
Observations:
\begin{itemize}
...
\begin{table}[h]
	...
	\begin{tabular}{|c||c|c|c|}
		\hline
		BJT mode & Gain/Offset & $I_S = \hat{f}_S (\Delta V)$ \\ \hline
