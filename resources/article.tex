...
The Class AB Amplifier (or \textit{Push-Pull}) is the typical output stage for many IC devices (op amps, audio amplifiers, etc.) when there is to drive a load with some power.
...
We assume that the base current are negligible. The collector currents are expressed as $I_C = f_S(V_{BE})$ for the NPN and $I_C = f_D(V_{EB})$ for the PNP. $V_B$ is usually around $1.4V$. $V_{IN}$ is the input voltage to amplify, we consider it is a fixed value. Then:
