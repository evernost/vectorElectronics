...
\subsubsection*{Method 1: the fixed diode voltage}
Assume the diode voltage is constant and $V_D = 0.7V$ (or up to a few Volts for LEDs). Set a target current, say $I_D = 5mA$, and determine $R$ accordingly:
$$
	R = \dfrac{V - V_D}{I_D}
$$

Done! This method yields results that are accurate enough for most applications.
...
\begin{table}[H]
	...
	\begin{tabular}{|m{1.5cm}||m{5cm}|m{6cm}|}
		\hline
		\textbf{BJT Mode} & \textbf{BJT Operating point} & \textbf{Rubber Diode} \\ \hline
...
The Differential Pair (or Long Tailed Pair ¬‿¬) is at the core of Operational Amplifiers.
...
The Ring Modulator is a special arrangement of diodes that, in certain conditions, produces a voltage that is the mathematical product of 2 input voltages. The term \textit{modulator} comes from the fact that this was the way signal modulation was achieved back in the days. This diode structure appeared in patents dating back from 1934\footnote{US Patent 2,025,158 \textit{Modulating System}, F. A. Cowan, 24 Dec 1934} (!). 

Notice how the structure slightly differs from the Full Bridge Rectifier (or \textit{Graetz Bridge}) 
...
\section*{Example 11: the diode Ladder Filter}
...
\section*{Example 12: the Moog Voltage Controlled High-Pass filter (module \textit{904B})}
