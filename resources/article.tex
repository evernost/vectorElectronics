...
\subsubsection*{Description}
The Moog Ladder Filter is a parametric low-pass filter used in audio applications.
...
In this section, we don't analyse the full schematic, only the 'top' part of the filter:

\begin{circuitikz}
	\draw (3, 3.98) -| (3, 0.5);
	\draw[line width=2pt, -latex] (3, 0.75) -- (3, 0.5);
	\node[shape=rectangle, minimum width=1.715cm, minimum height=0.985cm] at (1.875, 0.76){} node[anchor=north east, align=right, text width=1.327cm, inner sep=6pt] at (2.75, 1.27){$I_C + \dfrac{\Delta I}{2}$};
	...
	\draw (4.25, 6.75) -- (4.25, 7.75);
	\draw (2, 5.75) -| (4.25, 4.75);
	\node[circ] at (4.25, 6.75){};
	\node[ocirc](N3) at (4.25, 7.75){} node[anchor=south] at (N3.north){$V_{CC}$};
	\draw[dash pattern={on 1.6pt off 1.6pt}] (3, -1) -- (3, -1.77);
	\draw (5.5, 3.98) -| (5.5, 0.48);
	\draw (3, 3.25) -- (6.75, 3.25);
	\draw (3.84, 4.75) -- (4.66, 4.75);
	\draw[line width=2pt, -latex] (6, 3.25) -- (6.25, 3.25);
	\node[shape=rectangle, minimum width=1.965cm, minimum height=0.985cm] at (6.75, 0.72){} node[anchor=north east, align=right, text width=1.577cm, inner sep=6pt] at (7.75, 1.23){$I_C - \dfrac{\Delta I}{2}$};
	\node[circ] at (3, 3.25){};
	\node[circ] at (5.5, 1.75){};
	\draw (3, 0.5) to[ioosource] (3, -1);
	\draw (5.5, 0.48) to[ioosource] (5.5, -1.02);
	\node[ocirc](N4) at (6.75, 3.25){} node[anchor=west] at (N4.east){$V_P$};
	\draw[dash pattern={on 1.6pt off 1.6pt}] (5.5, -1.02) -- (5.5, -1.79);
	\draw[line width=2pt, -latex] (8, 2) -- (8, 3);
	\node[shape=rectangle, minimum width=0.715cm, minimum height=1.465cm] at (8.625, 2.5){} node[anchor=center, align=center, text width=0.327cm, inner sep=6pt] at (8.625, 2.5){$\Delta V$};
	\node[circ] at (4.25, 4.75){};
	\draw (3, 5.52) -| (3, 6.75) -- (5.5, 6.75) -| (5.5, 5.52);
	\draw (5.5, 1.75) -- (6.75, 1.75);
	\node[ocirc](N5) at (6.75, 1.75){} node[anchor=west] at (N5.east){$V_M$};
	\node[ocirc](N6) at (2, 5.75){} node[anchor=east] at (N6.west){$V_{BIAS}$};
	\draw[line width=2pt, -latex] (6.25, 1.75) -- (5.971, 1.75);
	\draw[line width=2pt, -latex] (5.5, 0.73) -- (5.5, 0.48);
	\node[shape=rectangle, minimum width=0.686cm, minimum height=0.465cm] at (6.14, 3.75){} node[anchor=west, align=left, text width=0.297cm, inner sep=6pt] at (5.779, 3.75){$I_P$};
	\node[shape=rectangle, minimum width=0.686cm, minimum height=0.465cm] at (6.11, 2.25){} node[anchor=west, align=left, text width=0.297cm, inner sep=6pt] at (5.75, 2.25){$I_M$};
	\draw[dash pattern={on 0.4pt off 1.6pt}] (7.5, 3.25) -- (9, 3.25);
	\draw[dash pattern={on 0.4pt off 1.6pt}] (7.5, 1.75) -- (9, 1.75);
\end{circuitikz}

To simplify, we assume here that the bottom section of the ladder is a balanced current source: two branches, a 'common mode' \textbf{control current} $I_C$ to which a 'differential mode' \textbf{signal current} $\frac{\Delta I}{2}$ is superimposed.
...
\left\{
	\begin{array}{ll}
		I_P = f(V_{BIAS}-V_P) - I_C - \frac{\Delta I_C}{2}\\
		I_M = -f(V_{BIAS}-V_M) + I_C - \frac{\Delta I_C}{2}
	\end{array}
\right.
$$\\
...
Put differently:
$$
\Delta V = -\dfrac{I + \Delta I}{a}
$$\\

If the load is a simple impedance $Z$, then $I = \frac{\Delta V}{Z}$ and it becomes:
$$
\Delta V = -\dfrac{Z}{2+aZ} \Delta I
$$\\

If we write that $a \triangleq \frac{1}{R_C}$ (the \textit{dynamic} resistance), we get this rather \textit{nice} expression:
$$
\Delta V = -\dfrac{1}{2} \left[ \dfrac{(2R_C) Z}{(2R_C) + Z} \right] \Delta I
$$\\

An observer looking at the impedance $Z$ would actually see a parallel association of the impedance $Z$ with some resistance $R_C$, with some $-1/2$ gain.
Recall that $\Delta I$ is a \textit{current} replica of the input signal to be filtered.
Now, it is key to remember that $a$ is the transconductance of the BJT in the model.
